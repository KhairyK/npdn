 ...
 * NPDN Worker v2.3 — patched: smart-resolve, pre-warm, meta, trace, gzip, canonical redirect
 * Powered by Cloudflare Workers — thanks to Cloudflare.
 */
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (pkgVer !== resolvedVersion && !wantIntegrityAll && !wantMeta && !filePath) {
        const canonicalPath = `/npm/${pkgName}@${resolvedVersion}/${filePath}`;
        const headers = new Headers();
        headers.set("Location", canonicalPath);
        // Also include Link rel=canonical so clients / crawlers see canonical URL
      ...
      // Serve the file (fresh)
      return await makeFileResponse(request, bodyUint8, filePath, false, Object.assign({}, headersExtra, { ETag: newETag, "Accept-Ranges": "bytes" }), trace);
  ...
  async scheduled(event, env, ctx) {
    ...
    for (const [pkg, ver, file] of POPULAR_PACKAGES) {
      const url = `https://npdn.kyrt.my.id/npm/${pkg}@${ver}/${file}`;
...
/* ------------------------------------------------------
   MIME + HEADERS -> RESPONSE (with optional gzip)
------------------------------------------------------ */
async function makeFileResponse(request, data, path, cached = false, extraHeaders = {}, trace = []) {
  const h = new Headers();
